...
class MatrizDispersa():
					...
					if aux.getCol()==col:
						...
						while aux!=None:
							if aux.getDato()==dia:
								...
								#Comprobar que la tabla no tenga un elemento igual
								aux.tablaHash.insertar(evento,direccion,hora,desc,col,fil)
								return 0
	...
	def insertar(self, col,fil,dia,evento,direccion,hora,desc):
		...
		nuevaCol=Nodo(col)
		nuevaFila=Nodo(fil)
		existencia=self.Si_Existe_Insertar_Dato(col,fil,dia,evento,direccion,hora,desc)
		...
		if existencia==1 or existencia==0:
			...
			if self.first == None:
				...
				dato.arriba=columna
				dato.ant=fila
				dato.tablaHash.insertar(evento,direccion,hora,desc,col,fil)
				...
				if int(aux.getCol())>int(col) and int(aux2.getDato())>int(col):
					...
					self.insertarAntesCol(nuevaCol,aux2)
					pass
				dato.tablaHash.insertar(evento)
	...
	def eliminar(self,dia,mes,anio,evento):
		...
		while aux!=None and not eliminado:
			print aux.getDato()+" va en el anioo----"
			if aux.getDato()==anio:
				print "ANIO ENCONTRADO... "+aux.getDato()
				aux=aux.abajo
				while aux!=None:
					print "NOW PRINTING ::: "+aux.getFi()
					if aux.getFi()==mes:
						print "MES ECNONTRADO::___ "+aux.getFi()
						if aux.getDato()==dia:
