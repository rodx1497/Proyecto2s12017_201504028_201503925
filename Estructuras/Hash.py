...
class TablaHash():
	...
	def insertaHash(self, arrHash, m, n,nombre,direccion,hora,desc,month,year):
		print "HASH222---"+month+" "+year
		correcto=False
		direccion=direccion+" "
...
class Hash():
	...
	def insertar(self,nombre,direccion,hora,desc,month,year):
		n=0
		for x in xrange(0,len(nombre)):
			n+=ord(nombre[x])
			pass
			print "HASH------- "+month+" "+year
		self.thash.insertaHash(self.arrHash,self.m,n,nombre,direccion,hora,desc,month,year)
